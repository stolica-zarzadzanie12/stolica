...
COMMENT ON COLUMN posts.author_id IS 'References the auth.users table';

-- Add explicit schema comment for PostgREST
COMMENT ON TABLE posts IS 'Blog posts table with foreign key to auth.users';
...
-- Create a view to handle the join
CREATE OR REPLACE VIEW posts_with_authors AS
SELECT 
  posts.*,
  auth.users.email as author_email
FROM posts
LEFT JOIN auth.users ON posts.author_id = auth.users.id;